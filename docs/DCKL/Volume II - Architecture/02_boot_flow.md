...
[Stage 6] Shell + AI
    ...
    │  luna-bar starts
    │  luna-island starts
    │  Ollama daemon starts
    │  luna-ai-d starts
    ▼
[Stage 7] Desktop Ready
       LUNA online signal sent to luna-island
       Session ready
```
...
| 4 | D-Bus alive | All Stage 4 services running | Mark degraded, continue |
| 5 | GPU/framebuffer ready | LGP compositor accepting connections | Fallback to framebuffer console |
| 6 | Compositor alive | Shell processes started | Degraded desktop (no AI) |
| 7 | Shell alive | luna-island shows LUNA online | AI degraded mode if luna-ai-d failed |
...
### Stage 6 — Shell and AI Layer

After confirming the LGP compositor is ready (method TBD — see Stage 5 TODO), `luna-init` starts shell components in this order:
...
5. `luna-ai-d` — LUNA.AI daemon (depends on Ollama being healthy)

`luna-ai-d` start sequence:
1. Verify Ollama is responding at `localhost:11434`
2. If Ollama is not yet ready: wait up to 10 seconds, polling every 500ms
3. If Ollama is unresponsive after 10 seconds: start `luna-ai-d` in degraded mode (local inference unavailable)
4. If Ollama is responsive: start `luna-ai-d` normally
5. `luna-ai-d` performs a self-test: loads personality engine, verifies `~/.luna/` structure
6. `luna-ai-d` signals readiness to `luna-island`

```
TODO:
Decision not yet finalized.
Reason: The IPC protocol from luna-ai-d to luna-island is not yet specified.
This must be decided as part of Volume IV architecture work.
```
...
| Stage 6 (shell + AI) | < 1.5s | 10s for Ollama |
...
[4100] [6] [luna-ai-d]      [INFO] LUNA online. Mode: AMBIENT
[4150] [7] [luna-init]      [INFO] Boot complete. Time: 4.15s. Status: NOMINAL
```
...
| Stage 6 Ollama timeout | luna-ai-d degraded mode | luna-island shows disconnected state |
| Stage 6 luna-ai-d failure | Desktop functional, AI unavailable | luna-island LUNA Amber |
...
- The graphics layer (Stage 5) starts the **LGP compositor** — not Wayland, not Hyprland, not any upstream compositor. If implementing this stage, the compositor process is a LunaOS-written component specified in Volume III.
...
- `/var/run/luna-boot-complete` is created by luna-init at Stage 7 completion. Services needing to detect boot completion should check for this file.
- `luna-ai-d` starting in degraded mode is a normal operating condition. The shell and desktop remain fully functional.
...
*Version: 0.2-draft*
*Depends on: architecture_overview.md, core_laws.md, decision_log.md (DL-002, DL-005, DL-009), non_negotiables.md*
